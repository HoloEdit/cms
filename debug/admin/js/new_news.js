    ...
    if(titolo == ""){
        $j("input#title").css('background-color','#F3D9D9');
         ...
         $j("input#title").css('background-color','');
          $j("#msg").hide()
          $j.post("new_news_funzione.php", { title: $j('#title').val(), img: $j('#img').val(), intro: $j('#news_intro').val(), completa: $j('#news_completa').val()}, function(risposta) {
     if (risposta == 1) {
   $j("#msg").fadeTo(200, 0.1, function() {
